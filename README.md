...
- **模型**：仅供 UI 手动会话使用，可选；留空时使用 Pi 默认配置。它不是 `$pi-codex:task` 参数。
...
`$pi-codex:task` 始终使用 Pi 自己的默认配置，不提供 `--model` 或 `--race`；Control Center 中手动新建的 UI 会话仍可选填模型。本仓库不保存提供商凭据，请使用 Pi 推荐的 `/login`、环境变量或 `~/.pi/agent/models.json` 配置方式。
