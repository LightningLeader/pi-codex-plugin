...
2. Resolve the plugin root: `PI_ROOT="${PI_PLUGIN_ROOT:-$HOME/pi-plugin-cc}"`.
3. Read the template: `cat "$PI_ROOT/plugins/pi/prompts/parallel-rescue.md"`.
...
5. Run: `node "$PI_ROOT/plugins/pi/scripts/pi-companion.mjs" task --write "<the constructed prompt>"`
