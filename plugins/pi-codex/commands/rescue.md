...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model>|--race <m1,m2,...>] [--effort <off|minimal|low|medium|high|xhigh|max>] [--out-file <path>] [what Pi should investigate, solve, or continue]"
allowed-tools: Bash(node:*), AskUserQuestion, Agent
---

Invoke the `pi-codex:pi-companion-forwarder` subagent via the `Agent` tool (`subagent_type: "pi-codex:pi-companion-forwarder"`), forwarding the raw user request as the prompt.
`pi-codex:pi-companion-forwarder` is a subagent, not a skill — do not call `Skill(pi-codex:pi-companion-forwarder)` (no such skill) or `Skill(pi:rescue)` (that re-enters this command and hangs the session). The command runs inline so the `Agent` tool stays in scope; forked general-purpose subagents do not expose it.
...
- If the request includes `--background`, run the `pi-codex:pi-companion-forwarder` subagent in the background.
- If the request includes `--wait`, run the `pi-codex:pi-companion-forwarder` subagent in the foreground.
...
- Do not ask the subagent to inspect files, monitor progress, poll `/pi-codex:status`, fetch `/pi-codex:result`, call `/pi-codex:cancel`, summarize output, or do follow-up work of its own.
...
- If the helper reports that Pi is missing or unconfigured, stop and tell the user to run `/pi-codex:setup`.
