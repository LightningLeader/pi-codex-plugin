...
- For `pi-codex:pi-companion-forwarder`, do not turn a failed or incomplete Pi run into a Claude-side implementation attempt. Report the failure and stop.
- For `pi-codex:pi-companion-forwarder`, if Pi was never successfully invoked, do not generate a substitute answer at all.
...
- If the helper reports that setup is required (pi binary missing, no provider configured), direct the user to `/pi-codex:setup` and do not improvise alternate flows.
